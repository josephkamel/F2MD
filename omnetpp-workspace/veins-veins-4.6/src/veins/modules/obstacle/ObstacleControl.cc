...
double ObstacleControl::calculateInsersion(const Coord& pos, double deltaX,
        double deltaY) {
    // calculate bounding box of transmission
    ...
    for (size_t col = fromCol; col <= toCol; ++col) {
        ...
        for (size_t row = fromRow; row <= toRow; ++row) {
            ...
            for (ObstacleGridCell::const_iterator k = cell.begin();
                ...
                Obstacle* o = *k;
                Coord p1(pos.x + deltaX, pos.y + deltaY);
                Coord p2(pos.x + deltaX, pos.y - deltaY);
                Coord p3(pos.x - deltaX, pos.y - deltaY);
                Coord p4(pos.x - deltaX, pos.y + deltaY);

                if (o->includesPos(pos)) {
                    pFailed[0] = 1;
                }
                if (o->includesPos(p1)) {
                    pFailed[1] = 1;
    ...
    double factor = 0;
    for (int var = 0; var < 5; ++var) {
        factor = factor + pFailed[var];
    }

    return factor;
}

double ObstacleControl::calculateInsersionList(const Coord& pos,
        double deltaX, double deltaY, Coord pList[], int pListSize) {
    // calculate bounding box of transmission
    Coord bboxP1 = Coord(std::min(pos.x - deltaX, pos.x + deltaX),
            std::min(pos.y - deltaY, pos.y + deltaY));
    Coord bboxP2 = Coord(std::max(pos.x - deltaX, pos.x + deltaX),
    ...
    std::set<Obstacle*> processedObstacles;
    double pFailed[pListSize];

    for (int var = 0; var < pListSize; ++var) {
        pFailed[var] = 0;
    }
